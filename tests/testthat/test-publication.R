test_that("publication helpers create reviewer-facing artifacts", {
  ...
  decision <- readiness_decision(readiness, diagnostics = NULL, checklist = checklist)
...
test_that("simulation study writes publication artifacts", {
  ...
  expect_true(nrow(study$readiness_decision) == 1L)
  ...
  expect_true(file.exists(file.path(output_dir, "readiness-decision.csv")))
